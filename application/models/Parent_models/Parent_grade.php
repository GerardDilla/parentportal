...
class Parent_grade extends CI_Model{
...
public function getSY(){
	...
	public function getGrades(){
		...
		if(isset($Sem) && isset($SY)){
				...
				$sql = 
				"
							SELECT B.Subject_Code,B.Prelim,B.Midterm,B.Finals,B.Final_Grade,C.`Remarks` FROM EnrolledStudent_Subjects AS A 
							JOIN Grading AS B 
							ON A.Sched_Code = B.Sched_Code
							JOIN Remarks AS C
							ON B.Remarks_ID = C.Remarks_ID
							WHERE B.Student_Number = '$Student_Number' 
							AND B.`School_Year` = '$SY' 
							AND B.`Semester` = '$Sem' 
							GROUP BY B.`Sched_Code`
				...
				if($result->num_rows() > 0){
						...
						foreach($result->result_array() as $row){
							...
							$sql2 = 
							"
								SELECT * FROM Subject WHERE Course_Code = '$SC' 
							...
							$result2 = $this->db->query($sql2);
						 	...
						 	foreach($result2->result_array() as $row2){
								 ...
								 $Output .= '<td>'.$row2['Course_Lec_Unit'].'</td>';
								 $Output .= '<td>'.$row2['Course_Lab_Unit'].'</td>';
				...
				"
					...
					ORDER BY `School_Year` DESC, `Semester` DESC LIMIT 1

				";
				$getnewresult = $this->db->query($getnew);

				foreach($getnewresult->result_array() as $get){
	...
	public function dashgrades(){
		...
		if($getnewresult->num_rows() > 0){
			...
			if($result->num_rows() > 0){
				...
				$Output .= "<br><br><h3>No records yet.</h3></br>";
